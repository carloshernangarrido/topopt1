...
class GUI(object):
    ...
    def __init__(self, problem, title=""):
        ...
        self.plot_fixed_nodes()
        # self.plot_passive_elements()
        self.fig.show()
    ...
    def plot_passive_elements(self):
    ...
    def plot_fixed_nodes(self):
        """Add green arrows to the plot for each fixed dof."""
        arrowprops = {"arrowstyle": "->", "connectionstyle": "arc3", "lw": "2",
                      "color": 'green', "mutation_scale": 25}
        fixed_nodes = self.problem.bc.fixed_nodes
        nelx, nely = (self.problem.nelx, self.problem.nely)
        for fixed_i in fixed_nodes:
            x, y = id_to_xy(fixed_i // 2, nelx, nely)
            x = max(min(x, nelx - 1), 0)
            y = max(min(y, nely - 1), 0)
            z = int(fixed_i % 2)
            mag = 1
